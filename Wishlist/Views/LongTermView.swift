...
struct LongTermView: View {
        ...
        NavigationStack {
            ZStack {
                ...
                VStack {
                    ...
                    NavigationLink(destination: InputView(settings: settings, type: .long)) {
                    ...
                    ScrollView {
                        LazyVStack(spacing: 0) {
                            ForEach(longItem, id: \.id) { item in
                                NavigationLink(destination: DetailView(settings: settings, longItem: item)) {
                                    itemRow(item: item)
                                }
                            }
                        }
    ...
    private func itemRow(item: LongItems) -> some View {
        HStack {
            HStack() {
                Text(item.name)
                    .font(.system(size: 18))
